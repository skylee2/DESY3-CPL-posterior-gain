#!/usr/bin/env python3
"""Reconstructed generator for manuscript Figure 2 (BR -> BRS geometry).
...
def generate(point_path: Path, output_path: Path, metadata_path: Path) -> dict[str, object]:
    ...
    weak_line = direction_segment(mu_ref, weak, 1.18)
    v1_line = direction_segment(mu_ref, v1, 1.18)
    v2_line = direction_segment(mu_ref, v2, 1.18)
    ...
    labels = [
        ...
        "BR mean",
        "BRS mean",
        "BR weak covariance axis",
        rf"dominant $v_1$ ($\Lambda_1={point['lambda1']:.2f}$; $\Delta\phi_{{\rm weak}}={angle_v1:.2f}^\circ$)",
        rf"second $v_2$ ($\Lambda_2={point['lambda2']:.2f}$)",
    ]
    axis.legend(handles, labels, loc="upper right", fontsize=9.2, framealpha=0.90)
    axis.set_title(r"BR $\rightarrow$ BRS: SN-added generalized covariance geometry")
    axis.set_xlabel(r"$w_0$")
    axis.set_ylabel(r"$w_a$")
    axis.set_xlim(-1.55, -0.25)
    axis.set_ylim(-2.35, 1.15)
    ...
    figure.savefig(
        ...
        bbox_inches="tight",
        metadata={
            "Title": "BR to BRS generalized covariance geometry",
...
def main() -> None:
    ...
    parser.add_argument(
        "--point", type=Path, default=PACKAGE_ROOT / "results" / "points" / "br_brs_point.json"
    )
    parser.add_argument(
        "--output", type=Path, default=PACKAGE_ROOT / "figures" / "fig1_BR_to_BRS_geometry_v06.pdf"
    )
    parser.add_argument(
        "--metadata", type=Path, default=PACKAGE_ROOT / "results" / "validation" / "figure2_metadata.json"
    )
