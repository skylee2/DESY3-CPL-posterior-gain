#!/usr/bin/env python3
"""Generate the manuscript-v3 D3 -> D3+BRS covariance-geometry figure."""
...
def covariance_ellipse(mean: np.ndarray, covariance: np.ndarray, linestyle: str) -> Ellipse:
...
def generate(point_path: Path, output_path: Path, metadata_path: Path) -> dict[str, object]:
    ...
    angle_v2 = acute_angle_deg(v2, weak)

    plt.rcParams.update({"font.size": 12, "axes.titlesize": 16, "axes.labelsize": 14})
    figure, axis = plt.subplots(figsize=(9.2, 7.6))
    axis.add_patch(covariance_ellipse(mu_ref, cov_ref, "-"))
    axis.add_patch(covariance_ellipse(mu_new, cov_new, "-."))
    axis.scatter(*mu_ref, marker="o", s=55, color="tab:blue", zorder=5)
    axis.scatter(*mu_new, marker="s", s=55, color="tab:orange", zorder=5)

    weak_line = direction_segment(mu_ref, weak, 1.12)
    v1_line = direction_segment(mu_ref, v1, 1.12)
    v2_line = direction_segment(mu_ref, v2, 1.12)
    axis.plot(*weak_line, "--", color="tab:blue", linewidth=2.0)
    axis.plot(*v1_line, ":", color="tab:green", linewidth=2.2)
    axis.plot(*v2_line, "-", color="tab:orange", linewidth=2.0)
    ...
    handles = [
        ...
        plt.Line2D([], [], linestyle=":", color="tab:green", linewidth=2.2),
        plt.Line2D([], [], linestyle="-", color="tab:orange", linewidth=2.0),
    ]
    labels = [
        "D3 = DES Y3 3×2pt (68%)",
        "D3+BRS (68%)",
        "D3 mean",
        "D3+BRS mean",
        "D3 weak covariance axis",
        rf"dominant $v_1$ ($\Lambda_1={point['lambda1']:.2f}$; $\Delta\phi_{{\rm weak}}={angle_v1:.2f}^\circ$)",
        rf"subdominant $v_2$ ($\Lambda_2={point['lambda2']:.2f}$; $\Delta\phi_{{\rm weak}}={angle_v2:.2f}^\circ$)",
    ]
    axis.legend(handles, labels, loc="upper right", fontsize=10.2, framealpha=0.88)
    axis.set_title(r"D3 $\rightarrow$ D3+BRS: generalized directions relative to D3 covariance axes")
    axis.set_xlabel(r"$w_0$")
    axis.set_ylabel(r"$w_a$")
    axis.set_xlim(-1.5, -0.3)
    axis.set_ylim(-2.5, 1.25)
    ...
    figure.savefig(
        ...
        bbox_inches="tight",
        metadata={
            "Title": "D3 to D3+BRS generalized directions relative to D3 covariance axes",
            "Author": "Seokcheon Lee",
            "Subject": (
                f"dominant v1 Lambda1={point['lambda1']:.8f}, weak-axis angle={angle_v1:.8f} deg; "
                f"subdominant v2 Lambda2={point['lambda2']:.8f}, weak-axis angle={angle_v2:.8f} deg"
            ),
            "CreationDate": None,
            "ModDate": None,
        },
    ...
    metadata = {
        ...
        "interpretation": "dominant v1 is not weak-axis aligned; subdominant v2 is weak-axis aligned",
        "Lambda1": float(point["lambda1"]),
        "Lambda2": float(point["lambda2"]),
        "v1_weak_axis_angle_deg": angle_v1,
        "v2_weak_axis_angle_deg": angle_v2,
...
def main() -> None:
    ...
    parser.add_argument(
        "--point", type=Path, default=PACKAGE_ROOT / "results" / "points" / "d3_d3brs_point.json"
    )
    parser.add_argument(
        "--output", type=Path, default=PACKAGE_ROOT / "figures" / "fig2_D3_to_D3BRS_geometry_v06.pdf"
    )
    parser.add_argument(
        "--metadata", type=Path, default=PACKAGE_ROOT / "results" / "validation" / "figure3_metadata.json"
    )
